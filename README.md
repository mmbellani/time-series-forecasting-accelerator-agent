...
- [Training on Time Series Forecasting and the Methodology Used](#training-on-time-series-forecasting-and-the-methodology-used)
  ...
  - [Profiling (clustering) Time Series:​](#profiling-clustering-time-series)
      ...
      - [Clustering profiles​](#clustering-profiles)
        ...
        - [Nixtla Forecasting Framework](#nixtla-forecasting-framework)
          - [Nixtla’s key design principle is:](#nixtlas-key-design-principle-is)
...
See [Pipeline Overview](docs/time-series-forecaster-design.md) for detailed documentation.

```
Raw Data → [01] Prep → [02] EDA → [03] Profile → [04] Cluster → [05] Features → [06] Train
```
...
| 02 | Exploratory Data Analysis | Explores Data |
| 03 | Profiling | Classify series (regular, lumpy, erratic, etc.) |
| 04 | Clustering | Group similar series with K-Means |
| 05 | Feature Engineering | Create lags, rolling stats, calendar features |
| 06 | Train/Tune | Train LightGBM, tune with Optuna |

## Requirements

- Microsoft Fabric workspace with Lakehouse
- Python packages: `mlforecast`, `lightgbm`, `optuna`, `xgboost`, see [requirements.txt](requirements.txt)

# Training on Time Series Forecasting and the Methodology Used
...
This accelerator provides you with 6 Notebooks that drives you through the essential steps you need to obtain a baseline forecast in a short amount of time.
...
Nixtla is an open‑source ecosystem of Python libraries for large-scale time series forecasting. Rather than a single framework, it’s a modular stack (“Nixtlaverse”) that supports statistical, machine learning, and deep learning approaches in a unified way.

###### Nixtla’s key design principle is:

Standardise forecasting workflows across many time series with scalable, production‑ready components

This makes it particularly strong for:

Global modelling (many series at once)
Automation (feature engineering, evaluation, tuning)
Scalability (Spark / Dask / Ray)

###### Main Components
The ecosystem is composed of specialised libraries:

- StatsForecast → fast statistical models (ARIMA, ETS, Theta, etc.)
- MLForecast → ML-based forecasting (LightGBM, XGBoost, sklearn models)
- NeuralForecast → deep learning models (N-BEATS, NHITS, Transformers)
- HierarchicalForecast → hierarchical & grouped time series
- TimeGPT → pretrained foundation models for forecasting

## Functions Overview
Very brief overview of the functions that are available in the Notebooks.
