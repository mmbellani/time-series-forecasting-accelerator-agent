"""End-to-end example: intermittent-demand forecasting + LightGBM benchmark.
...
Every intermittent model is run twice — GLOBALLY over the entire panel and BY
...
sys.path.append(str(Path(__file__).resolve().parents[1]))  # skills/intermittent-forecasting
...
def run_in_notebook(panel_df, forecasts_df, save_dir=None):
    # 0) Build the Croston-family model set (add naive baselines for context)
    models = build_models(add_naive_baselines=False)

    # 1) GLOBAL run — entire panel in one StatsForecast backtest
    print("=== Global intermittent backtest ===")
    ...
    print("\n=== Per-cluster intermittent backtest ===")
    ...
    print("\nBest intermittent model per cluster:")
    ...
    print("\n=== Global: Intermittent vs LightGBM ===")
    ...
    print("\n=== Per-cluster: Intermittent vs LightGBM ===")
    ...
    if save_dir:
        ...
        res_global["metrics"].to_csv(out / f"{SCENARIO}_intermittent_global.csv", index=False)
        res_cluster["metrics"].to_csv(out / f"{SCENARIO}_intermittent_by_cluster.csv", index=False)
